...
docker run --rm --network host -v "$ROOT_DIR":/work -w /work grafana/k6:latest run performance-tests/k6/baseline.js --out json=performance-tests/results/baseline.json

echo "Running spike..."
docker run --rm --network host -v "$ROOT_DIR":/work -w /work grafana/k6:latest run performance-tests/k6/spike.js --out json=performance-tests/results/spike.json

echo "Running stress..."
docker run --rm --network host -v "$ROOT_DIR":/work -w /work grafana/k6:latest run performance-tests/k6/stress.js --out json=performance-tests/results/stress.json
